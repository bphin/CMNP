...
class Spectrum(object):
    ...
    def Plot_Spectrum_En(self):
        ...
        for i, conf in enumerate(field_kwargs):
            ...
            plt.plot(1.241285500000001e3/wl, spectrum.T[0], 'g-', label='ext.')
            plt.plot(1.241285500000001e3/wl, spectrum.T[1], 'b-', label='scat.')
            plt.plot(1.241285500000001e3/wl, spectrum.T[2], 'r-', label='abs.')
    ...
    def find_maxima(self, prominence=0.05, distance=5):
        
        if not isinstance(self.wl, np.ndarray):
            raise ValueError("self.wl is not a valid NumPy array.")
        
        if not isinstance(self.spectrum, np.ndarray):
            raise ValueError("self.spectrum is not a valid NumPy array.")

        extinction = self.spectrum.T[0]
        scattering = self.spectrum.T[1]
        absorption = self.spectrum.T[2]

        peaks_ext, _ = find_peaks(extinction, prominence=prominence, distance=distance)
        peaks_scat, _ = find_peaks(scattering, prominence=prominence, distance=distance)
        peaks_abs, _ = find_peaks(absorption, height=0.15*np.max(Spectrum),prominence=0.3*np.max(Spectrum),distance=15, width=5, wlen=100)
                                  
        print("Extinction Maxima:", self.wl[peaks_ext])
        print("Scattering Maxima:", self.wl[peaks_scat])
        print("Absorption Maxima:", self.wl[peaks_abs])

        # Plot the spectra with detected peaks
        plt.figure(figsize=(8, 5))
        plt.plot(self.wl, extinction, 'g-', label='Extinction')
        plt.plot(self.wl, scattering, 'b-', label='Scattering')
        plt.plot(self.wl, absorption, 'r-', label='Absorption')

        # Mark peaks
        plt.scatter(self.wl[peaks_ext], extinction[peaks_ext], color='green', marker='o', label='Ext. Peaks')
        plt.scatter(self.wl[peaks_scat], scattering[peaks_scat], color='blue', marker='o', label='Scat. Peaks')
        plt.scatter(self.wl[peaks_abs], absorption[peaks_abs], color='red', marker='o', label='Abs. Peaks')
